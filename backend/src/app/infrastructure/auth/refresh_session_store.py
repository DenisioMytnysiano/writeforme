...
class RefreshSessionStoreProtocol(Protocol):
    ...
    def get_session_for_user(self, email: str, fingerprint: str) -> Optional[RefreshSession]:
        pass
...
class RefreshSessionStore:
    ...
    def get_session_for_user(self, email: str, fingerprint: str) -> Optional[RefreshSession]:
        return self.__db_session.query(RefreshSession) \
            .where(RefreshSession.email == email and RefreshSession.fingerprint == fingerprint) \
            .first()

    def delete_and_get_session(self, refresh_token: str) -> Optional[RefreshSession]:
        session = self.__db_session.query(RefreshSession) \
