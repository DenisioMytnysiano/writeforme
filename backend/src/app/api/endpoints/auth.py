...
def register(user: RegisterRequest, auth_service: AuthServiceProtocol = Depends(AuthService)):
    return auth_service.register(user.email, user.password)


@router.post("/refresh-token", response_model=TokenResponse)
def refresh_token(response: Response, refresh_request: RefreshTokenRequest, tokens: AuthTokenPair = Depends(jwtTokenBearer),auth_service: AuthServiceProtocol = Depends(AuthService)):
    token_pair = auth_service.refresh_token(tokens, refresh_request.fingerprint)
    response.set_cookie(key="refresh_token", value=f"Bearer {token_pair.refresh_token}", path="/auth", httponly=True)
    return token_pair

@router.post("/logout")
def logout(tokens: AuthTokenPair = Depends(jwtTokenBearer), auth_service: AuthServiceProtocol = Depends(AuthService)):
